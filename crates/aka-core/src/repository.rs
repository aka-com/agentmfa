//! Durable broker-state boundaries.
//!
//! The local product still persists through [`crate::store::Store`].  These
//! interfaces describe the broker operations rather than that store's JSON
//! layout, so a hosted implementation can perform the same operations in a
//! database transaction.  Cached reads remain synchronous: they are on every
//! agent request today, and a remote implementation is expected to maintain a
//! workspace-scoped read model rather than turn one capability call into a
//! sequence of database round trips.  Operations which may durably mutate
//! state are asynchronous.

use uuid::Uuid;

use crate::audit::{AuditEntry, AuditIntegrity, AuditPage};
use crate::endpoints::{EndpointRegistry, IssuedEndpoint};
use crate::health::HealthRegistry;
use crate::identity::{IdentityStore, ManageTokenMutationError, TokenError, VerifiedToken};
use crate::onepassword::{OnePasswordAuth, OnePasswordIntegration, OnePasswordSecretRef};
use crate::policy::AccessTable;
use crate::store::{ConnectionSpec, NewCredential, PinOutcome, Store};
use crate::template::Template;
use crate::types::{
    BrokerIdentity, ConfirmMode, Connection, ConnectionHealth, ConnectionKind, DirectEndpoint,
    HealthStatus, SecretMeta, SecretValue, Settings, ToolAccess,
};
use crate::Result;

/// Trusted tenant scope for every durable repository operation.
///
/// Request handlers must obtain this from authenticated broker state rather
/// than constructing it from a request field.  Keeping it as an opaque value
/// makes an unscoped repository call impossible and gives a hosted backend one
/// consistent key for queries, transactions, cache entries, and audit data.
#[derive(Clone, Debug, PartialEq, Eq, Hash)]
pub struct WorkspaceContext {
    workspace_id: Uuid,
}

impl WorkspaceContext {
    /// Scope repositories to a hosted workspace selected by trusted code.
    pub fn new(workspace_id: Uuid) -> Self {
        Self { workspace_id }
    }

    /// The one workspace owned by the local broker.
    pub fn local() -> Self {
        Self::new(Uuid::nil())
    }

    pub fn workspace_id(&self) -> Uuid {
        self.workspace_id
    }
}

/// The read model needed by request execution and management rendering.
///
/// Implementations may serve these methods from an in-process snapshot.  The
/// returned values are owned so a refresh can replace that snapshot without
/// tying callers to repository locks.
#[async_trait::async_trait]
pub trait CatalogReader: Send + Sync {
    fn retired_connections_dropped(&self, workspace: &WorkspaceContext) -> Vec<String>;
    fn list_secrets(&self, workspace: &WorkspaceContext) -> Vec<SecretMeta>;
    fn secret_by_id(&self, workspace: &WorkspaceContext, id: &Uuid) -> Result<SecretMeta>;
    fn secret_by_name(&self, workspace: &WorkspaceContext, name: &str) -> Option<SecretMeta>;
    async fn secret_totp_code(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
    ) -> Result<(String, u64)>;
    async fn secret_value(&self, workspace: &WorkspaceContext, id: &Uuid) -> Result<SecretValue>;
    async fn secret_value_by_name(
        &self,
        workspace: &WorkspaceContext,
        name: &str,
    ) -> Result<SecretValue>;

    fn list_onepassword_integrations(
        &self,
        workspace: &WorkspaceContext,
    ) -> Vec<OnePasswordIntegration>;
    fn onepassword_integration(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
    ) -> Result<OnePasswordIntegration>;
    fn invalidate_onepassword_integration(&self, workspace: &WorkspaceContext, id: &Uuid);
    async fn validate_new_onepassword_integration(
        &self,
        workspace: &WorkspaceContext,
        id: Uuid,
        label: &str,
        auth: &OnePasswordAuth,
        token: Option<&str>,
    ) -> Result<()>;
    async fn validate_onepassword_replacement_token(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
        token: &str,
    ) -> Result<()>;
    async fn onepassword_health(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
    ) -> Result<aka_api::OnePasswordHealthDto>;
    async fn onepassword_vaults(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
    ) -> Result<Vec<aka_api::OnePasswordVaultDto>>;
    async fn onepassword_items(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
        vault_id: &str,
    ) -> Result<Vec<aka_api::OnePasswordItemDto>>;
    async fn onepassword_fields(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
        vault_id: &str,
        item_id: &str,
    ) -> Result<Vec<aka_api::OnePasswordFieldDto>>;
    async fn render_template(
        &self,
        workspace: &WorkspaceContext,
        template: &Template,
    ) -> Result<SecretValue>;

    fn connections_using(&self, workspace: &WorkspaceContext, id: &Uuid) -> Vec<String>;
    fn list_connections(&self, workspace: &WorkspaceContext) -> Vec<Connection>;
    fn connection_by_id(&self, workspace: &WorkspaceContext, id: &Uuid) -> Result<Connection>;
    fn connection_by_name(&self, workspace: &WorkspaceContext, name: &str) -> Option<Connection>;
    fn preflight_add_connection(
        &self,
        workspace: &WorkspaceContext,
        spec: &ConnectionSpec,
    ) -> Result<()>;
    fn preflight_add_connection_with_secret(
        &self,
        workspace: &WorkspaceContext,
        secret_name: &str,
        spec: &ConnectionSpec,
    ) -> Result<()>;
    async fn connection_oauth_grant(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
    ) -> Result<SecretValue>;
    fn settings(&self, workspace: &WorkspaceContext) -> Settings;
}

/// Atomic durable catalog operations.
///
/// Methods intentionally correspond to domain operations rather than table
/// writes. In particular, secret rename owns template rewrites and combined
/// secret/connection creation stays one call.
///
/// Every method in this trait is a transaction boundary: an implementation
/// must durably commit the complete domain operation before returning `Ok`,
/// and must leave both its durable state and read model unchanged on `Err`.
/// A future Postgres implementation can therefore open and close one database
/// transaction per method without leaking SQL transaction handles upward.
#[async_trait::async_trait]
pub trait CatalogRepository: CatalogReader {
    async fn add_secret(
        &self,
        workspace: &WorkspaceContext,
        name: &str,
        value: SecretValue,
    ) -> Result<SecretMeta>;
    async fn add_credential(
        &self,
        workspace: &WorkspaceContext,
        spec: NewCredential,
    ) -> Result<SecretMeta>;
    async fn set_password_profile(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
        site: Option<String>,
        username: Option<Option<String>>,
    ) -> Result<SecretMeta>;
    async fn set_totp_factor(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
        seed: Option<SecretValue>,
    ) -> Result<SecretMeta>;
    async fn rename_secret(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
        new_name: &str,
    ) -> Result<(SecretMeta, usize)>;
    async fn replace_secret_value(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
        value: SecretValue,
    ) -> Result<SecretMeta>;
    async fn delete_secret(&self, workspace: &WorkspaceContext, id: &Uuid) -> Result<SecretMeta>;

    async fn add_onepassword_integration_with_id(
        &self,
        workspace: &WorkspaceContext,
        id: Uuid,
        label: &str,
        auth: OnePasswordAuth,
        token: Option<SecretValue>,
    ) -> Result<OnePasswordIntegration>;
    async fn replace_onepassword_token(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
        token: SecretValue,
    ) -> Result<OnePasswordIntegration>;
    async fn delete_onepassword_integration(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
    ) -> Result<OnePasswordIntegration>;
    async fn add_onepassword_secret(
        &self,
        workspace: &WorkspaceContext,
        name: &str,
        reference: OnePasswordSecretRef,
    ) -> Result<SecretMeta>;

    async fn add_connection(
        &self,
        workspace: &WorkspaceContext,
        spec: ConnectionSpec,
    ) -> Result<Connection>;
    async fn add_connection_with_secret(
        &self,
        workspace: &WorkspaceContext,
        secret_name: &str,
        value: SecretValue,
        spec: ConnectionSpec,
    ) -> Result<(SecretMeta, Connection)>;
    async fn update_connection(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
        spec: ConnectionSpec,
    ) -> Result<(Connection, bool)>;
    async fn update_connection_if_current(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
        expected_updated_at: &str,
        spec: ConnectionSpec,
    ) -> Result<(Connection, bool)>;
    async fn rename_connection_if_current(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
        expected_updated_at: &str,
        name: String,
    ) -> Result<Connection>;
    async fn rename_connection(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
        name: String,
    ) -> Result<Connection>;
    async fn set_connection_account(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
        account: Option<String>,
    ) -> Result<Connection>;
    async fn set_connection_oauth(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
        payload: SecretValue,
        expires_at: Option<chrono::DateTime<chrono::Utc>>,
    ) -> Result<Connection>;
    async fn pin_ssh_host_key(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
        observed: &ssh_key::Fingerprint,
    ) -> Result<PinOutcome>;
    async fn delete_connection(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
    ) -> Result<Connection>;
    async fn reorder_connections(
        &self,
        workspace: &WorkspaceContext,
        ordered_ids: &[Uuid],
    ) -> Result<Vec<Connection>>;
    async fn set_menu_bar_hides_dock(&self, workspace: &WorkspaceContext, on: bool) -> Result<()>;
    async fn set_confirm_ssh_host_keys(&self, workspace: &WorkspaceContext, on: bool)
        -> Result<()>;
}

#[async_trait::async_trait]
impl CatalogReader for Store {
    fn retired_connections_dropped(&self, _workspace: &WorkspaceContext) -> Vec<String> {
        Store::retired_connections_dropped(self).to_vec()
    }

    fn list_secrets(&self, _workspace: &WorkspaceContext) -> Vec<SecretMeta> {
        Store::list_secrets(self)
    }

    fn secret_by_id(&self, _workspace: &WorkspaceContext, id: &Uuid) -> Result<SecretMeta> {
        Store::secret_by_id(self, id)
    }

    fn secret_by_name(&self, _workspace: &WorkspaceContext, name: &str) -> Option<SecretMeta> {
        Store::secret_by_name(self, name)
    }

    async fn secret_totp_code(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
    ) -> Result<(String, u64)> {
        Store::secret_totp_code(self, id).await
    }

    async fn secret_value(&self, _workspace: &WorkspaceContext, id: &Uuid) -> Result<SecretValue> {
        Store::secret_value(self, id).await
    }

    async fn secret_value_by_name(
        &self,
        _workspace: &WorkspaceContext,
        name: &str,
    ) -> Result<SecretValue> {
        Store::secret_value_by_name(self, name).await
    }

    fn list_onepassword_integrations(
        &self,
        _workspace: &WorkspaceContext,
    ) -> Vec<OnePasswordIntegration> {
        Store::list_onepassword_integrations(self)
    }

    fn onepassword_integration(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
    ) -> Result<OnePasswordIntegration> {
        Store::onepassword_integration(self, id)
    }

    fn invalidate_onepassword_integration(&self, _workspace: &WorkspaceContext, id: &Uuid) {
        Store::invalidate_onepassword_integration(self, id)
    }

    async fn validate_new_onepassword_integration(
        &self,
        _workspace: &WorkspaceContext,
        id: Uuid,
        label: &str,
        auth: &OnePasswordAuth,
        token: Option<&str>,
    ) -> Result<()> {
        Store::validate_new_onepassword_integration(self, id, label, auth, token).await
    }

    async fn validate_onepassword_replacement_token(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        token: &str,
    ) -> Result<()> {
        Store::validate_onepassword_replacement_token(self, id, token).await
    }

    async fn onepassword_health(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
    ) -> Result<aka_api::OnePasswordHealthDto> {
        Store::onepassword_health(self, id).await
    }

    async fn onepassword_vaults(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
    ) -> Result<Vec<aka_api::OnePasswordVaultDto>> {
        Store::onepassword_vaults(self, id).await
    }

    async fn onepassword_items(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        vault_id: &str,
    ) -> Result<Vec<aka_api::OnePasswordItemDto>> {
        Store::onepassword_items(self, id, vault_id).await
    }

    async fn onepassword_fields(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        vault_id: &str,
        item_id: &str,
    ) -> Result<Vec<aka_api::OnePasswordFieldDto>> {
        Store::onepassword_fields(self, id, vault_id, item_id).await
    }

    async fn render_template(
        &self,
        _workspace: &WorkspaceContext,
        template: &Template,
    ) -> Result<SecretValue> {
        Store::render_template(self, template).await
    }

    fn connections_using(&self, _workspace: &WorkspaceContext, id: &Uuid) -> Vec<String> {
        Store::connections_using(self, id)
    }

    fn list_connections(&self, _workspace: &WorkspaceContext) -> Vec<Connection> {
        Store::list_connections(self)
    }

    fn connection_by_id(&self, _workspace: &WorkspaceContext, id: &Uuid) -> Result<Connection> {
        Store::connection_by_id(self, id)
    }

    fn connection_by_name(&self, _workspace: &WorkspaceContext, name: &str) -> Option<Connection> {
        Store::connection_by_name(self, name)
    }

    fn preflight_add_connection(
        &self,
        _workspace: &WorkspaceContext,
        spec: &ConnectionSpec,
    ) -> Result<()> {
        Store::preflight_add_connection(self, spec)
    }

    fn preflight_add_connection_with_secret(
        &self,
        _workspace: &WorkspaceContext,
        secret_name: &str,
        spec: &ConnectionSpec,
    ) -> Result<()> {
        Store::preflight_add_connection_with_secret(self, secret_name, spec)
    }

    async fn connection_oauth_grant(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
    ) -> Result<SecretValue> {
        Store::connection_oauth_grant(self, id).await
    }

    fn settings(&self, _workspace: &WorkspaceContext) -> Settings {
        Store::settings(self)
    }
}

#[async_trait::async_trait]
impl CatalogRepository for Store {
    async fn add_secret(
        &self,
        _workspace: &WorkspaceContext,
        name: &str,
        value: SecretValue,
    ) -> Result<SecretMeta> {
        Store::add_secret(self, name, value)
    }

    async fn add_credential(
        &self,
        _workspace: &WorkspaceContext,
        spec: NewCredential,
    ) -> Result<SecretMeta> {
        Store::add_credential(self, spec)
    }

    async fn set_password_profile(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        site: Option<String>,
        username: Option<Option<String>>,
    ) -> Result<SecretMeta> {
        Store::set_password_profile(self, id, site, username)
    }

    async fn set_totp_factor(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        seed: Option<SecretValue>,
    ) -> Result<SecretMeta> {
        Store::set_totp_factor(self, id, seed)
    }

    async fn rename_secret(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        new_name: &str,
    ) -> Result<(SecretMeta, usize)> {
        Store::rename_secret(self, id, new_name)
    }

    async fn replace_secret_value(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        value: SecretValue,
    ) -> Result<SecretMeta> {
        Store::replace_secret_value(self, id, value)
    }

    async fn delete_secret(&self, _workspace: &WorkspaceContext, id: &Uuid) -> Result<SecretMeta> {
        Store::delete_secret(self, id)
    }

    async fn add_onepassword_integration_with_id(
        &self,
        _workspace: &WorkspaceContext,
        id: Uuid,
        label: &str,
        auth: OnePasswordAuth,
        token: Option<SecretValue>,
    ) -> Result<OnePasswordIntegration> {
        Store::add_onepassword_integration_with_id(self, id, label, auth, token)
    }

    async fn replace_onepassword_token(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        token: SecretValue,
    ) -> Result<OnePasswordIntegration> {
        Store::replace_onepassword_token(self, id, token)
    }

    async fn delete_onepassword_integration(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
    ) -> Result<OnePasswordIntegration> {
        Store::delete_onepassword_integration(self, id)
    }

    async fn add_onepassword_secret(
        &self,
        _workspace: &WorkspaceContext,
        name: &str,
        reference: OnePasswordSecretRef,
    ) -> Result<SecretMeta> {
        Store::add_onepassword_secret(self, name, reference)
    }

    async fn add_connection(
        &self,
        _workspace: &WorkspaceContext,
        spec: ConnectionSpec,
    ) -> Result<Connection> {
        Store::add_connection(self, spec)
    }

    async fn add_connection_with_secret(
        &self,
        _workspace: &WorkspaceContext,
        secret_name: &str,
        value: SecretValue,
        spec: ConnectionSpec,
    ) -> Result<(SecretMeta, Connection)> {
        Store::add_connection_with_secret(self, secret_name, value, spec)
    }

    async fn update_connection(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        spec: ConnectionSpec,
    ) -> Result<(Connection, bool)> {
        Store::update_connection(self, id, spec)
    }

    async fn update_connection_if_current(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        expected_updated_at: &str,
        spec: ConnectionSpec,
    ) -> Result<(Connection, bool)> {
        Store::update_connection_if_current(self, id, expected_updated_at, spec)
    }

    async fn rename_connection_if_current(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        expected_updated_at: &str,
        name: String,
    ) -> Result<Connection> {
        Store::rename_connection_if_current(self, id, expected_updated_at, name)
    }

    async fn rename_connection(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        name: String,
    ) -> Result<Connection> {
        Store::rename_connection(self, id, name)
    }

    async fn set_connection_account(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        account: Option<String>,
    ) -> Result<Connection> {
        Store::set_connection_account(self, id, account)
    }

    async fn set_connection_oauth(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        payload: SecretValue,
        expires_at: Option<chrono::DateTime<chrono::Utc>>,
    ) -> Result<Connection> {
        Store::set_connection_oauth(self, id, payload, expires_at)
    }

    async fn pin_ssh_host_key(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        observed: &ssh_key::Fingerprint,
    ) -> Result<PinOutcome> {
        Store::pin_ssh_host_key(self, id, observed)
    }

    async fn delete_connection(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
    ) -> Result<Connection> {
        Store::delete_connection(self, id)
    }

    async fn reorder_connections(
        &self,
        _workspace: &WorkspaceContext,
        ordered_ids: &[Uuid],
    ) -> Result<Vec<Connection>> {
        Store::reorder_connections(self, ordered_ids)
    }

    async fn set_menu_bar_hides_dock(&self, _workspace: &WorkspaceContext, on: bool) -> Result<()> {
        Store::set_menu_bar_hides_dock(self, on)
    }

    async fn set_confirm_ssh_host_keys(
        &self,
        _workspace: &WorkspaceContext,
        on: bool,
    ) -> Result<()> {
        Store::set_confirm_ssh_host_keys(self, on)
    }
}

/// Per-connection policy, scoped to one workspace.
///
/// Each async setter is an atomic domain transaction. Implementations must
/// update the durable policy and its read model together, or change neither.
#[async_trait::async_trait]
pub trait PolicyRepository: Send + Sync {
    fn allows(&self, workspace: &WorkspaceContext, connection_id: &Uuid) -> bool;
    fn allowed_tools(
        &self,
        workspace: &WorkspaceContext,
        connection_id: &Uuid,
    ) -> Option<Vec<String>>;
    fn confirm_mode(&self, workspace: &WorkspaceContext, connection_id: &Uuid) -> ConfirmMode;
    fn expose_response_credentials(
        &self,
        workspace: &WorkspaceContext,
        connection_id: &Uuid,
    ) -> bool;
    fn audit_statements(
        &self,
        workspace: &WorkspaceContext,
        connection_id: &Uuid,
        default: bool,
    ) -> bool;
    fn entry(&self, workspace: &WorkspaceContext, connection_id: &Uuid) -> Option<ToolAccess>;
    fn entries(&self, workspace: &WorkspaceContext) -> Vec<ToolAccess>;

    async fn set_enabled(
        &self,
        workspace: &WorkspaceContext,
        connection_id: Uuid,
        enabled: bool,
    ) -> Result<bool>;
    async fn set_allowed_tools(
        &self,
        workspace: &WorkspaceContext,
        connection_id: Uuid,
        tools: Option<Vec<String>>,
    ) -> Result<bool>;
    async fn set_confirm_mode(
        &self,
        workspace: &WorkspaceContext,
        connection_id: Uuid,
        confirm: ConfirmMode,
    ) -> Result<bool>;
    async fn set_expose_response_credentials(
        &self,
        workspace: &WorkspaceContext,
        connection_id: Uuid,
        expose: bool,
    ) -> Result<bool>;
    async fn set_audit_statements(
        &self,
        workspace: &WorkspaceContext,
        connection_id: Uuid,
        audit_statements: Option<bool>,
    ) -> Result<bool>;
    async fn remove_for_connection(
        &self,
        workspace: &WorkspaceContext,
        connection_id: &Uuid,
    ) -> Result<bool>;
}

#[async_trait::async_trait]
impl PolicyRepository for AccessTable {
    fn allows(&self, _workspace: &WorkspaceContext, connection_id: &Uuid) -> bool {
        AccessTable::allows(self, connection_id)
    }

    fn allowed_tools(
        &self,
        _workspace: &WorkspaceContext,
        connection_id: &Uuid,
    ) -> Option<Vec<String>> {
        AccessTable::allowed_tools(self, connection_id)
    }

    fn confirm_mode(&self, _workspace: &WorkspaceContext, connection_id: &Uuid) -> ConfirmMode {
        AccessTable::confirm_mode(self, connection_id)
    }

    fn expose_response_credentials(
        &self,
        _workspace: &WorkspaceContext,
        connection_id: &Uuid,
    ) -> bool {
        AccessTable::expose_response_credentials(self, connection_id)
    }

    fn audit_statements(
        &self,
        _workspace: &WorkspaceContext,
        connection_id: &Uuid,
        default: bool,
    ) -> bool {
        AccessTable::audit_statements(self, connection_id, default)
    }

    fn entry(&self, _workspace: &WorkspaceContext, connection_id: &Uuid) -> Option<ToolAccess> {
        AccessTable::entry(self, connection_id)
    }

    fn entries(&self, _workspace: &WorkspaceContext) -> Vec<ToolAccess> {
        AccessTable::entries(self)
    }

    async fn set_enabled(
        &self,
        _workspace: &WorkspaceContext,
        connection_id: Uuid,
        enabled: bool,
    ) -> Result<bool> {
        AccessTable::set_enabled(self, connection_id, enabled)
    }

    async fn set_allowed_tools(
        &self,
        _workspace: &WorkspaceContext,
        connection_id: Uuid,
        tools: Option<Vec<String>>,
    ) -> Result<bool> {
        AccessTable::set_allowed_tools(self, connection_id, tools)
    }

    async fn set_confirm_mode(
        &self,
        _workspace: &WorkspaceContext,
        connection_id: Uuid,
        confirm: ConfirmMode,
    ) -> Result<bool> {
        AccessTable::set_confirm_mode(self, connection_id, confirm)
    }

    async fn set_expose_response_credentials(
        &self,
        _workspace: &WorkspaceContext,
        connection_id: Uuid,
        expose: bool,
    ) -> Result<bool> {
        AccessTable::set_expose_response_credentials(self, connection_id, expose)
    }

    async fn set_audit_statements(
        &self,
        _workspace: &WorkspaceContext,
        connection_id: Uuid,
        audit_statements: Option<bool>,
    ) -> Result<bool> {
        AccessTable::set_audit_statements(self, connection_id, audit_statements)
    }

    async fn remove_for_connection(
        &self,
        _workspace: &WorkspaceContext,
        connection_id: &Uuid,
    ) -> Result<bool> {
        AccessTable::remove_for_connection(self, connection_id)
    }
}

/// Authentication material for one workspace.
#[async_trait::async_trait]
pub trait IdentityRepository: Send + Sync {
    fn client_id(&self, workspace: &WorkspaceContext) -> Uuid;
    fn token(&self, workspace: &WorkspaceContext) -> String;
    fn info(&self, workspace: &WorkspaceContext) -> BrokerIdentity;
    fn active_alias_count(&self, workspace: &WorkspaceContext) -> usize;
    fn verify(
        &self,
        workspace: &WorkspaceContext,
        token: &str,
    ) -> std::result::Result<VerifiedToken, TokenError>;
    fn verify_manage(
        &self,
        workspace: &WorkspaceContext,
        token: &str,
    ) -> std::result::Result<(), TokenError>;
    fn manage_token_issued(&self, workspace: &WorkspaceContext) -> bool;
    fn manage_token_expires_at(
        &self,
        workspace: &WorkspaceContext,
    ) -> Option<chrono::DateTime<chrono::Utc>>;

    async fn touch(&self, workspace: &WorkspaceContext);
    async fn issue_manage_token(&self, workspace: &WorkspaceContext) -> Result<String> {
        self.issue_manage_token_with_ttl(workspace, None).await
    }
    async fn issue_manage_token_with_ttl(
        &self,
        workspace: &WorkspaceContext,
        ttl: Option<std::time::Duration>,
    ) -> Result<String>;
    async fn rotate_manage_token_with_ttl(
        &self,
        workspace: &WorkspaceContext,
        current: &str,
        ttl: Option<std::time::Duration>,
    ) -> std::result::Result<String, ManageTokenMutationError>;
    async fn revoke_manage_token(&self, workspace: &WorkspaceContext) -> Result<bool>;
    async fn revoke_manage_token_with_token(
        &self,
        workspace: &WorkspaceContext,
        current: &str,
    ) -> std::result::Result<(), ManageTokenMutationError>;
    async fn rotate(&self, workspace: &WorkspaceContext) -> Result<String>;
}

#[async_trait::async_trait]
impl IdentityRepository for IdentityStore {
    fn client_id(&self, _workspace: &WorkspaceContext) -> Uuid {
        IdentityStore::client_id(self)
    }

    fn token(&self, _workspace: &WorkspaceContext) -> String {
        IdentityStore::token(self)
    }

    fn info(&self, _workspace: &WorkspaceContext) -> BrokerIdentity {
        IdentityStore::info(self)
    }

    fn active_alias_count(&self, _workspace: &WorkspaceContext) -> usize {
        IdentityStore::active_alias_count(self)
    }

    fn verify(
        &self,
        _workspace: &WorkspaceContext,
        token: &str,
    ) -> std::result::Result<VerifiedToken, TokenError> {
        IdentityStore::verify(self, token)
    }

    fn verify_manage(
        &self,
        _workspace: &WorkspaceContext,
        token: &str,
    ) -> std::result::Result<(), TokenError> {
        IdentityStore::verify_manage(self, token)
    }

    fn manage_token_issued(&self, _workspace: &WorkspaceContext) -> bool {
        IdentityStore::manage_token_issued(self)
    }

    fn manage_token_expires_at(
        &self,
        _workspace: &WorkspaceContext,
    ) -> Option<chrono::DateTime<chrono::Utc>> {
        IdentityStore::manage_token_expires_at(self)
    }

    async fn touch(&self, _workspace: &WorkspaceContext) {
        IdentityStore::touch(self)
    }

    async fn issue_manage_token_with_ttl(
        &self,
        _workspace: &WorkspaceContext,
        ttl: Option<std::time::Duration>,
    ) -> Result<String> {
        IdentityStore::issue_manage_token_with_ttl(self, ttl)
    }

    async fn rotate_manage_token_with_ttl(
        &self,
        _workspace: &WorkspaceContext,
        current: &str,
        ttl: Option<std::time::Duration>,
    ) -> std::result::Result<String, ManageTokenMutationError> {
        IdentityStore::rotate_manage_token_with_ttl(self, current, ttl)
    }

    async fn revoke_manage_token(&self, _workspace: &WorkspaceContext) -> Result<bool> {
        IdentityStore::revoke_manage_token(self)
    }

    async fn revoke_manage_token_with_token(
        &self,
        _workspace: &WorkspaceContext,
        current: &str,
    ) -> std::result::Result<(), ManageTokenMutationError> {
        IdentityStore::revoke_manage_token_with_token(self, current)
    }

    async fn rotate(&self, _workspace: &WorkspaceContext) -> Result<String> {
        IdentityStore::rotate(self)
    }
}

/// Standing direct-endpoint registry.
///
/// Each async method is an atomic domain transaction for the supplied
/// workspace; no partially updated endpoint may reach the read model.
#[async_trait::async_trait]
pub trait EndpointRepository: Send + Sync {
    fn list(&self, workspace: &WorkspaceContext) -> Vec<DirectEndpoint>;
    fn get(&self, workspace: &WorkspaceContext, id: &Uuid) -> Option<DirectEndpoint>;
    fn get_for_connection(
        &self,
        workspace: &WorkspaceContext,
        connection_id: &Uuid,
    ) -> Option<DirectEndpoint>;
    fn resolve_secret(
        &self,
        workspace: &WorkspaceContext,
        presented: &str,
    ) -> Option<DirectEndpoint>;

    async fn issue(
        &self,
        workspace: &WorkspaceContext,
        connection_id: Uuid,
        kind: ConnectionKind,
    ) -> Result<IssuedEndpoint>;
    async fn renew(&self, workspace: &WorkspaceContext, id: &Uuid) -> Result<DirectEndpoint>;
    async fn set_expiry(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
        expire: bool,
    ) -> Result<DirectEndpoint>;
    async fn set_require_auth(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
        require_auth: bool,
    ) -> Result<bool>;
    async fn set_port(&self, workspace: &WorkspaceContext, id: &Uuid, port: u16) -> Result<()>;
    async fn revoke(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
    ) -> Result<Option<DirectEndpoint>>;
    async fn revoke_all(&self, workspace: &WorkspaceContext) -> Result<Vec<DirectEndpoint>>;
    async fn remove_for_connection(
        &self,
        workspace: &WorkspaceContext,
        connection_id: &Uuid,
    ) -> Result<Vec<DirectEndpoint>>;
}

#[async_trait::async_trait]
impl EndpointRepository for EndpointRegistry {
    fn list(&self, _workspace: &WorkspaceContext) -> Vec<DirectEndpoint> {
        EndpointRegistry::list(self)
    }

    fn get(&self, _workspace: &WorkspaceContext, id: &Uuid) -> Option<DirectEndpoint> {
        EndpointRegistry::get(self, id)
    }

    fn get_for_connection(
        &self,
        _workspace: &WorkspaceContext,
        connection_id: &Uuid,
    ) -> Option<DirectEndpoint> {
        EndpointRegistry::get_for_connection(self, connection_id)
    }

    fn resolve_secret(
        &self,
        _workspace: &WorkspaceContext,
        presented: &str,
    ) -> Option<DirectEndpoint> {
        EndpointRegistry::resolve_secret(self, presented)
    }

    async fn issue(
        &self,
        _workspace: &WorkspaceContext,
        connection_id: Uuid,
        kind: ConnectionKind,
    ) -> Result<IssuedEndpoint> {
        EndpointRegistry::issue(self, connection_id, kind)
    }

    async fn renew(&self, _workspace: &WorkspaceContext, id: &Uuid) -> Result<DirectEndpoint> {
        EndpointRegistry::renew(self, id)
    }

    async fn set_expiry(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        expire: bool,
    ) -> Result<DirectEndpoint> {
        EndpointRegistry::set_expiry(self, id, expire)
    }

    async fn set_require_auth(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        require_auth: bool,
    ) -> Result<bool> {
        EndpointRegistry::set_require_auth(self, id, require_auth)
    }

    async fn set_port(&self, _workspace: &WorkspaceContext, id: &Uuid, port: u16) -> Result<()> {
        EndpointRegistry::set_port(self, id, port)
    }

    async fn revoke(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
    ) -> Result<Option<DirectEndpoint>> {
        EndpointRegistry::revoke(self, id)
    }

    async fn revoke_all(&self, _workspace: &WorkspaceContext) -> Result<Vec<DirectEndpoint>> {
        EndpointRegistry::revoke_all(self)
    }

    async fn remove_for_connection(
        &self,
        _workspace: &WorkspaceContext,
        connection_id: &Uuid,
    ) -> Result<Vec<DirectEndpoint>> {
        EndpointRegistry::remove_for_connection(self, connection_id)
    }
}

/// Durable activity storage. Appends stay non-failing to preserve the current
/// operational-log contract; implementations that perform network I/O should
/// enqueue them and make delivery health observable separately.
#[async_trait::async_trait]
pub trait AuditRepository: Send + Sync {
    fn append(&self, workspace: &WorkspaceContext, entry: AuditEntry);
    async fn clear(&self, workspace: &WorkspaceContext) -> Result<()>;
    async fn verify(&self, workspace: &WorkspaceContext) -> AuditIntegrity;
    async fn recent(&self, workspace: &WorkspaceContext, limit: usize) -> Vec<AuditEntry>;
    async fn recent_page(
        &self,
        workspace: &WorkspaceContext,
        limit: usize,
        before: Option<u64>,
    ) -> AuditPage;
}

#[async_trait::async_trait]
impl AuditRepository for crate::audit::AuditLog {
    fn append(&self, _workspace: &WorkspaceContext, entry: AuditEntry) {
        crate::audit::AuditLog::append(self, entry)
    }

    async fn clear(&self, _workspace: &WorkspaceContext) -> Result<()> {
        crate::audit::AuditLog::clear(self)
    }

    async fn verify(&self, _workspace: &WorkspaceContext) -> AuditIntegrity {
        crate::audit::AuditLog::verify(self)
    }

    async fn recent(&self, _workspace: &WorkspaceContext, limit: usize) -> Vec<AuditEntry> {
        crate::audit::AuditLog::recent(self, limit)
    }

    async fn recent_page(
        &self,
        _workspace: &WorkspaceContext,
        limit: usize,
        before: Option<u64>,
    ) -> AuditPage {
        crate::audit::AuditLog::recent_page(self, limit, before)
    }
}

/// Last-observed upstream health. Writes are best-effort telemetry, not part
/// of the configuration transaction.
pub trait HealthRepository: Send + Sync {
    fn was_discarded(&self, workspace: &WorkspaceContext) -> bool;
    fn get(&self, workspace: &WorkspaceContext, id: &Uuid) -> Option<ConnectionHealth>;
    fn record(&self, workspace: &WorkspaceContext, id: &Uuid, status: HealthStatus, detail: String);
    fn record_ok_if_changed(&self, workspace: &WorkspaceContext, id: &Uuid, detail: String);
    fn record_credential_rejection(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
        detail: String,
    ) -> bool;
    fn record_if_changed(
        &self,
        workspace: &WorkspaceContext,
        id: &Uuid,
        status: HealthStatus,
        detail: String,
    );
    fn clear_rejection_streak(&self, workspace: &WorkspaceContext, id: &Uuid);
    fn forget(&self, workspace: &WorkspaceContext, id: &Uuid);
}

impl HealthRepository for HealthRegistry {
    fn was_discarded(&self, _workspace: &WorkspaceContext) -> bool {
        HealthRegistry::was_discarded(self)
    }

    fn get(&self, _workspace: &WorkspaceContext, id: &Uuid) -> Option<ConnectionHealth> {
        HealthRegistry::get(self, id)
    }

    fn record(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        status: HealthStatus,
        detail: String,
    ) {
        HealthRegistry::record(self, id, status, detail)
    }

    fn record_ok_if_changed(&self, _workspace: &WorkspaceContext, id: &Uuid, detail: String) {
        HealthRegistry::record_ok_if_changed(self, id, detail)
    }

    fn record_credential_rejection(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        detail: String,
    ) -> bool {
        HealthRegistry::record_credential_rejection(self, id, detail)
    }

    fn record_if_changed(
        &self,
        _workspace: &WorkspaceContext,
        id: &Uuid,
        status: HealthStatus,
        detail: String,
    ) {
        HealthRegistry::record_if_changed(self, id, status, detail)
    }

    fn clear_rejection_streak(&self, _workspace: &WorkspaceContext, id: &Uuid) {
        HealthRegistry::clear_rejection_streak(self, id)
    }

    fn forget(&self, _workspace: &WorkspaceContext, id: &Uuid) {
        HealthRegistry::forget(self, id)
    }
}

#[cfg(test)]
mod tests {
    use std::sync::Arc;

    use zeroize::Zeroizing;

    use super::*;
    use crate::integrity::StateIntegrity;
    use crate::paths::Paths;
    use crate::types::{ConnectionConfig, PgSslMode};
    use crate::vault::MemoryVault;

    #[tokio::test]
    async fn local_catalog_satisfies_the_async_repository_contract() {
        let dir = tempfile::tempdir().unwrap();
        let store: Arc<dyn CatalogRepository> = Arc::new(
            Store::open(Paths::under(dir.path()), Arc::new(MemoryVault::new()))
                .await
                .unwrap(),
        );
        let workspace = WorkspaceContext::local();

        let secret = store
            .add_secret(
                &workspace,
                "DATABASE_PASSWORD",
                Zeroizing::new("first".into()),
            )
            .await
            .unwrap();
        assert_eq!(
            store
                .secret_by_name(&workspace, "DATABASE_PASSWORD")
                .unwrap()
                .id,
            secret.id
        );
        store
            .replace_secret_value(&workspace, &secret.id, Zeroizing::new("second".into()))
            .await
            .unwrap();
        assert_eq!(
            &*store.secret_value(&workspace, &secret.id).await.unwrap(),
            "second"
        );

        let connection = store
            .add_connection(
                &workspace,
                ConnectionSpec {
                    name: "warehouse".into(),
                    config: ConnectionConfig::Pg {
                        host: "db.example.com".into(),
                        port: 5432,
                        dbname: "app".into(),
                        user: "app".into(),
                        sslmode: PgSslMode::VerifyFull,
                        trusted_ca_bundle_path: None,
                    },
                    secrets: vec![secret.id],
                },
            )
            .await
            .unwrap();
        assert_eq!(
            store
                .connection_by_id(&workspace, &connection.id)
                .unwrap()
                .name,
            "warehouse"
        );
    }

    #[tokio::test]
    async fn local_catalog_honors_domain_transaction_boundaries() {
        let dir = tempfile::tempdir().unwrap();
        let store: Arc<dyn CatalogRepository> = Arc::new(
            Store::open(Paths::under(dir.path()), Arc::new(MemoryVault::new()))
                .await
                .unwrap(),
        );
        let workspace = WorkspaceContext::local();

        let (secret, connection) = store
            .add_connection_with_secret(
                &workspace,
                "API_TOKEN",
                Zeroizing::new("first".into()),
                ConnectionSpec {
                    name: "api".into(),
                    config: ConnectionConfig::Api {
                        host: "api.example.com".into(),
                        scheme: "https".into(),
                        port: None,
                        trusted_ca_bundle_path: None,
                        template: "Authorization: Bearer {{API_TOKEN}}".into(),
                        mcp_path: None,
                        test_path: None,
                        oauth: None,
                        signer: None,
                        client_cert_path: None,
                        client_key_path: None,
                    },
                    secrets: Vec::new(),
                },
            )
            .await
            .unwrap();
        assert_eq!(
            store
                .connection_by_id(&workspace, &connection.id)
                .unwrap()
                .secrets,
            [secret.id]
        );

        let (_, rewritten) = store
            .rename_secret(&workspace, &secret.id, "RENAMED_TOKEN")
            .await
            .unwrap();
        assert_eq!(rewritten, 1);
        let renamed_connection = store.connection_by_id(&workspace, &connection.id).unwrap();
        let ConnectionConfig::Api { template, .. } = &renamed_connection.config else {
            unreachable!()
        };
        assert_eq!(template, "Authorization: Bearer {{RENAMED_TOKEN}}");

        store
            .add_secret(&workspace, "TAKEN_TOKEN", Zeroizing::new("other".into()))
            .await
            .unwrap();
        assert!(store
            .rename_secret(&workspace, &secret.id, "TAKEN_TOKEN")
            .await
            .is_err());
        assert_eq!(
            store.secret_by_id(&workspace, &secret.id).unwrap().name,
            "RENAMED_TOKEN"
        );
        assert_eq!(
            store
                .connection_by_id(&workspace, &connection.id)
                .unwrap()
                .config,
            renamed_connection.config
        );

        let stale_version = connection.version();
        let mut updated_spec = ConnectionSpec {
            name: renamed_connection.name.clone(),
            config: renamed_connection.config.clone(),
            secrets: renamed_connection.secrets.clone(),
        };
        let ConnectionConfig::Api { host, .. } = &mut updated_spec.config else {
            unreachable!()
        };
        *host = "new.example.com".into();
        assert!(store
            .update_connection_if_current(
                &workspace,
                &connection.id,
                &stale_version,
                updated_spec.clone(),
            )
            .await
            .is_err());
        assert_eq!(
            store
                .connection_by_id(&workspace, &connection.id)
                .unwrap()
                .config,
            renamed_connection.config
        );

        store
            .update_connection_if_current(
                &workspace,
                &connection.id,
                &renamed_connection.version(),
                updated_spec,
            )
            .await
            .unwrap();
        assert!(matches!(
            store
                .connection_by_id(&workspace, &connection.id)
                .unwrap()
                .config,
            ConnectionConfig::Api { host, .. } if host == "new.example.com"
        ));
    }

    #[tokio::test]
    async fn local_policy_identity_and_endpoints_are_object_safe() {
        let dir = tempfile::tempdir().unwrap();
        let paths = Paths::under(dir.path());
        paths.ensure().unwrap();
        let vault = Arc::new(MemoryVault::new());
        let integrity = Arc::new(StateIntegrity::open(vault.as_ref()).await.unwrap());

        let policy: Arc<dyn PolicyRepository> =
            Arc::new(AccessTable::open(paths.access_file(), integrity.clone()).unwrap());
        let workspace = WorkspaceContext::local();
        let connection_id = Uuid::new_v4();
        assert!(policy
            .set_enabled(&workspace, connection_id, false)
            .await
            .unwrap());
        assert!(!policy.allows(&workspace, &connection_id));

        let endpoints: Arc<dyn EndpointRepository> =
            Arc::new(EndpointRegistry::open(paths.endpoints_file(), 8, integrity.clone()).unwrap());
        let issued = endpoints
            .issue(&workspace, connection_id, ConnectionKind::Pg)
            .await
            .unwrap();
        assert_eq!(
            endpoints
                .get_for_connection(&workspace, &connection_id)
                .unwrap()
                .id,
            issued.endpoint.id
        );

        let identity: Arc<dyn IdentityRepository> = Arc::new(
            IdentityStore::open(
                paths.identity_file(),
                paths.token_file(),
                None,
                std::time::Duration::from_secs(60),
                integrity,
            )
            .unwrap(),
        );
        let manage_token = identity.issue_manage_token(&workspace).await.unwrap();
        assert!(identity.verify_manage(&workspace, &manage_token).is_ok());
    }
}
