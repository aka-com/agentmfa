...
returned by default; a connection can explicitly contain them at the broker
boundary. When returned, `Set-Cookie` is the exception to lossy combining: its
distinct values are also preserved in `set_cookie_headers`; use that array
rather than the combined `headers["set-cookie"]` value.
...
/// A generated skill anchored to setup text fetched from a selected broker.
/// The short setup remains useful when checked in: it tells the agent how to
/// fetch that broker's live, authoritative instructions at use time.
...
mod tests {
    ...
    #[test]
    fn broker_skill_embeds_authoritative_setup_without_claiming_a_local_socket() {
