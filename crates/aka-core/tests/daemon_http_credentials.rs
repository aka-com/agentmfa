    ...
    futures::executor::block_on(async {
        h.broker
            ...
            .add_secret(
                &h.broker.workspace,
                "AWS_ACCESS_KEY_ID",
                Zeroizing::new(AWS_ACCESS_KEY.into()),
            )
            .await
            .unwrap();
        h.broker
...
async fn gcp_service_account_tokens_are_minted_injected_and_scrubbed() {
    ...
    h.broker
        .store
        .add_secret(&h.broker.workspace, "GCP_SA_KEY", Zeroizing::new(key_json))
        .await
        .unwrap();
    h.broker
        .store
...
async fn authorization_is_reserved_even_for_a_query_form_connection() {
    ...
    h.broker
        ...
        .add_secret(
            &h.broker.workspace,
            "STREAM_TOKEN",
            Zeroizing::new("tok_abcdef123456".into()),
        )
        .await
        .unwrap();
    h.broker
...
async fn private_api_ca_is_honored_and_tls_failures_are_classified() {
    ...
    let trusted = h
        .broker
        .store
        .connection_by_name(&h.broker.workspace, "private-api")
        .unwrap();
    ...
    let untrusted = h
        .broker
        .store
        .connection_by_name(&h.broker.workspace, "untrusted-api")
        .unwrap();
...
async fn a_render_failure_flips_health_on_a_non_oauth_connection() {
    ...
    let secret = h
        .broker
        .store
        .secret_by_name(&h.broker.workspace, "GITHUB_API_KEY")
        .unwrap();
    h.broker
        .store
...
fn oauth_connection(h: &Harness, upstream_port: u16, token_port: u16, refresh: Option<&str>) {
    futures::executor::block_on(async {
        ...
        let secret = h
            .broker
            .store
            .secret_by_name(&h.broker.workspace, "OAUTH_TOKENS")
            .unwrap();
        h.broker
            .store
            .add_connection(
...
async fn endpoint_for(h: &Harness, name: &str) -> (String, String) {
    let id = h
        .broker
        .store
        .connection_by_name(&h.broker.workspace, name)
        .unwrap()
        .id;
...
async fn the_endpoint_secret_is_recoverable_from_the_vault_after_a_reload() {
    ...
    let id = broker
        .store
        .connection_by_name(&broker.workspace, "github")
        .unwrap()
        .id;
    let issued = broker.ui_issue_endpoint(&id).await.unwrap();
    let endpoint_id = broker
        .endpoints
        .get_for_connection(&broker.workspace, &id)
        .expect("issued")
        .id;
