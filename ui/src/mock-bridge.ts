...
// Standalone-browser development adapter. This module must stay behind
      ...
      return {
        ...
        endpoint: record?.endpoint
          ? {
              ...
              // No deadline (expiry off) carries no remaining seconds.
              expires_in_secs: record.endpoint.expires_at
                ? Math.max(
                    0,
...
async function mockInvoke(cmd: CommandName, args: MockArgs): Promise<unknown> {
  switch (cmd) {
    ...
    case 'issue_endpoint': {
      ...
      // Expiry is opt-in: a fresh endpoint has no deadline ('' on the wire).
    ...
    case 'get_endpoint': {
      ...
      return {
        ...
        expires_in_secs: endpoint.expires_at
          ? Math.max(
              0,
